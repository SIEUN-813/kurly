...
export default function Sub7AdminIdSearchComponent() {
    ...
    const [state, setState] = React.useState({
        ...
        isEmail: false,
        name: '',
        isNameGuideText: false,
        phone: '',
        isPhoneGuideText: false,
        receiveAuthen: null, // 발급된인증번호 
        hpAuthen: '', // 입력한인증번호
        email: '',
        isEmailGuideText: false,
        isEnable1: false,
        isEnable2: false,
        isAuthenNumberHpBox: false, // 인증번호
        isAuthenNumberEmailBox: false, // 인증번호
    })
    ...
    const onchangeName=(e)=>{
        let isNameGuideText = false;
        if(e.target.value === ''){
            isNameGuideText = true;
        }
        else{
            isNameGuideText = false;
        }
        setState({
            ...state,
            name: e.target.value,
            isNameGuideText: isNameGuideText
        })
    }
    ...
    React.useEffect(()=>{
        const regexp1 = /^01[0-9]{1}[0-9]{3,4}[0-9]{4}$/g; 
        let isEnable1 = false;
        if(regexp1.test(state.phone) === true && state.name !== ''){
            isEnable1 = true;
        }
        else{
            isEnable1 = false;
    ...
    React.useEffect(()=>{
        const regexp1 = /^[A-Za-z0-9가-힣ㄱ-ㅎㅏ-ㅣ`~!#$%^&*\-_+={}|'?]+((\.)?[A-Za-z0-9가-힣ㄱ-ㅎㅏ-ㅣ`~!#$%^&*\-_+={}|'?]+)*@[A-Za-z0-9가-힣ㄱ-ㅎㅏ-ㅣ`~!#$%^&*\-_+={}|'?.]+\.[A-Za-z0-9가-힣ㄱ-ㅎㅏ-ㅣ`~!#$%^&*\-_+={}|'?.]+$/g;
        let isEnable2 = false;
        if(regexp1.test(state.email) === true && state.name !== ''){
            isEnable2 = true;
        }
        else{
            isEnable2 = false;
    ...
    const onSubmitHpAuthen=(e)=>{
        ...
        axios({
            url: 'http://pse990813.dothome.co.kr/kurly_green/kurly_admin_id_search_name_hp_select.php',
            method: 'POST',
            data: formData
        })
        .then((res)=>{
    ...
    const onClickOkBtn=(e)=>{
        ...
        if(Number(state.hpAuthen) === state.receiveAuthen){
            // 키보드로 인증번호 입력 후 => 인증번호 입력상자 아래 확인 버튼 클릭 => 발급된 인증번호와 비교 => 조회결과컴포넌트로 전송
            navigate('/sub7AdminIdPwSearchResult',
                {                  
                    state: {
                        id: state.id,
            ...
            <section id="section1">
                <div className="container">
                    <div className="title">
                        ...
                        <h3 className='title-sub-txt' style={title2}>아이디 찾기</h3>
                    </div>
                    <div className="content sub6-content">
                            ...
                            <form id='hpAuthen' autoComplete='off' onSubmit={onSubmitHpAuthen}>
                                <ul>
                                    <li>
                                        ...
                                        <p className={`guide-text${state.isNameGuideText ? ' on' : ''}`}>가입 시 등록한 이름을 입력해주세요</p>
                                    </li>
                                    <li>
                                        <div className="gap">  
                        ...
                        }
                        {
                            state.isEmail &&
                            (                        
                            <form id='emailAuthen' autoComplete='off' onSubmit={onSubmitEmailAuthen}>
                                <ul>
                                    <li>
                                        <div className="gap">
                                            <label htmlFor="userName">이름</label>        
                                    ...
                                    {
                                        state.isAuthenNumberEmailBox &&
                                            (                                                                        
                                            <li>
                                                <div className="gap authen-number">  
                                                    <label htmlFor="userName">인증번호</label>     
                                                    <div className="box">                                      
                                                        <input 
                                                            type="text"
                                                            name='userHp'
